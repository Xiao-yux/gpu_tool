"""Tests for negpu.config.loader (TOML + search order + version merge + first-run)."""
...
from negpu.config.loader import (
...
def test_find_config_file_env_var_wins(
    ...
    env_cfg = tmp_path / "custom" / "negpu.toml"
    env_cfg.parent.mkdir(parents=True)
    env_cfg.write_text("version = '0.1.0'\n", encoding="utf-8")
    monkeypatch.setenv("NEGPU_CONFIG", str(env_cfg))
...
def test_load_config_missing_keys_get_defaults(
    ...
    monkeypatch.setattr("negpu.config.loader._SEARCH_PATHS", (str(cfg),))
    ...
    assert config.system.lto is True
...
def test_install_default_config_resolves_user_placeholder(
    ...
    monkeypatch.setattr("negpu.config.loader.current_user", lambda: "alice")
...
def test_load_returns_negpu_config(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    cfg = tmp_path / "config.toml"
    cfg.write_text("[log]\nlog_level = \"WARNING\"\n", encoding="utf-8")
    monkeypatch.setattr("negpu.config.loader._SEARCH_PATHS", (str(cfg),))
    result = NegpuConfig.model_validate(
    ...
    from negpu.config.loader import load
    assert isinstance(load(), NegpuConfig)
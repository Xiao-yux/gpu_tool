"""negpu.config.loader - TOML config loader + first-run install.

Implements REFACTORING_TASK [1]B / [2]C / [4]A / [14]C:

* [1]  Config file is named ``config.toml``.
* [2]  Search order: ``/etc/negpu/config.toml`` → ``~/config.toml`` →
       ``./config.toml`` → ``$NEGPU_CONFIG``.
* [4]  Version mismatch → deep-merge defaults INTO the user file
       (缺键补默认，旧值保留) so the user's edits are preserved.
* [14]  On the very first run (no ``/etc/negpu/config.toml``) the
...
from negpu.config.paths import current_user
...
_SEARCH_PATHS: Final[tuple[str, ...]] = (
    "/etc/negpu/config.toml",
    "~/config.toml",
    "./config.toml",
)

# The canonical install location for the first-run copy.
_DEFAULT_INSTALL_PATH: Final[str] = "/etc/negpu/config.toml"
...
# Re-running ``negpu config-install`` will not overwrite your changes.
...
[paths]
config_file  = "/etc/negpu/config.toml"
...
bash_dir     = "/usr/local/share/negpu/bash"
...
def find_config_file() -> Path | None:
    ...
    1. ``/etc/negpu/config.toml``
    2. ``~/config.toml``
    3. ``./config.toml``
    4. ``$NEGPU_CONFIG``  (if set)
    """
    for raw in _SEARCH_PATHS:
        p = Path(raw).expanduser()
        if p.is_file():
            return p
    env = os.environ.get("NEGPU_CONFIG")
    if env:
...
def load_config(
    ...
    *,
...
) -> tuple[NegpuConfig, Path]:
    """Load the negpu configuration.
    ...
    ``/etc/negpu/config.toml`` so the user can edit it next time.
    """
    if explicit_path is not None:
        ...
        raw = _read_toml(path)
        return NegpuConfig.model_validate(raw), path
    ...
    if found is not None:
        ...
        return NegpuConfig.model_validate(merged), found
    ...
    if install_if_missing and sys.platform.startswith("linux"):
        try:
            ...
            return NegpuConfig.model_validate(raw), installed
        except (PermissionError, OSError):
            # Not root or /etc not writable → silently fall back to in-memory defaults.
            pass

    return NegpuConfig(), Path("(defaults)")


def _default_dump() -> dict[str, object]:
    """Dump the built-in :class:`NegpuConfig` defaults to a plain dict."""
    return NegpuConfig().model_dump()


# Re-export the main entry point under a shorter name for callers.
def load() -> NegpuConfig:

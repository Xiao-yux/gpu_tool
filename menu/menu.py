...
class Menu:
    ...
    def main_menu(self):
        """主菜单"""
        pro = ListPrompt("请选择操作:",choices=self.menu_chess.main_menu,validator=lambda x: x != self.menu_chess.main_menu[0],error_message="暂未完成").prompt()
        if pro.data == "exit":
            os._exit(0)
        elif pro.data == "2":
            self.sys_info_menu()
    ...
    def system_set_menu(self):
        """BMC用户设置菜单"""
        pro = ListPrompt("请选择BMC用户设置项:",choices=self.menu_chess.setsystem_menu,validator=lambda x: x != self.menu_chess.setsystem_menu[1],error_message="暂未完成").prompt()
        if pro.data == "exit":
            self.main_menu()
        elif pro.data == "1":
    ...
    def sys_info_menu(self):
        ...
        if pro.data == "1":
            ...
            self.log.msg(self.tool.run_command("nvidia-smi topo -m"),outconsole=True)
        elif pro.data == "5":
            self.log.msg(self.tool.run_command("ipmitool lan print"),outconsole=True)
        self.log.msg(f'用户选择: {pro}')

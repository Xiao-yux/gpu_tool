"""Tests for gpu_tool.config.paths (path resolver + LogPaths)."""
...
from gpu_tool.config.paths import (
    LogPaths,
    REPORT_SUBDIR,
    RUN_SUBDIR,
    SCRIPT_SUBDIR,
...
def test_user_log_root_expands_user(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("gpu_tool.config.paths.current_user", lambda: "alice")
...
def test_user_log_root_substitutes_user(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("gpu_tool.config.paths.current_user", lambda: "bob")
    root = user_log_root("/var/log/gpu_tool/{user}")
...
def test_serial_number_falls_back_to_hostname(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("gpu_tool.config.paths.shutil.which", lambda _x: None)
    sn = serial_number()
...
def test_serial_number_sanitises_unsafe_chars(
    ...
    monkeypatch.setattr("gpu_tool.config.paths.shutil.which", lambda _x: "/bin/yes")
    monkeypatch.setattr(
        "gpu_tool.config.paths.subprocess_run_capture", lambda _a: "  abc/def 123 "
    )
    sn = serial_number()
    assert "/" not in sn
    assert " " not in sn
...
def test_serial_number_rejects_known_garbage(
    ...
    monkeypatch.setattr("gpu_tool.config.paths.shutil.which", lambda _x: "/bin/yes")
    monkeypatch.setattr(
        "gpu_tool.config.paths.subprocess_run_capture", lambda _a: "To be filled by O.E.M."
    )
    sn = serial_number()

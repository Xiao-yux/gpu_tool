...
from gpu_tool.config.model import (
    LanguageConfig,
    LogConfig,
    gpuConfig,
    PathConfig,
...
def test_path_config_does_not_have_main_file() -> None:
    ...
    assert "main_file" not in gpuConfig().paths.model_dump()
...
def test_with_overrides_replaces_top_level_field() -> None:
    c = gpuConfig().with_overrides(version="9.9.9")
...
def test_with_overrides_replaces_dotted_path() -> None:
    c = gpuConfig().with_overrides(**{"log.log_level": "DEBUG"})
...
def test_with_overrides_replaces_full_section() -> None:
    ...
    c = gpuConfig().with_overrides(log=new_log)
...
def test_extra_keys_are_ignored() -> None:
    ...
    c = gpuConfig.model_validate(raw)
...
def test_negpu_config_dump_round_trip() -> None:
    ...
    reloaded = gpuConfig.model_validate(dumped)
